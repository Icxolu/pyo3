...
use pyo3_macros_backend::{
    build_derive_cloneref, build_derive_from_pyobject, build_py_class, build_py_enum,
...
#[proc_macro_derive(CloneRef, attributes(pyo3))]
pub fn derive_clone_ref(item: TokenStream) -> TokenStream {
    let ast = parse_macro_input!(item as syn::DeriveInput);
    let expanded = build_derive_cloneref(&ast).unwrap_or_compile_error();
    quote!(
        #expanded
    )
    .into()
